    ...
    if request.method == "POST":
        ...
        try:
            ...
            if user_id:
                ...
                flash("Registration failed due to an unknown error. Please try again or contact support.", "error")
                return render_template("register.html")
        except mysql.connector.Error as db_err:
            flash(f"Database error: {db_err.msg}", "error")
            return render_template("register.html")
        except Exception as e:
...
def latest_news():
    ...
    from enhanced_stock_info import get_comprehensive_stock_info
    stock_info = get_comprehensive_stock_info(company)
...
def get_sentiment(symbol):
    ...
    """
    result = analyze_sentiment_only(symbol)
